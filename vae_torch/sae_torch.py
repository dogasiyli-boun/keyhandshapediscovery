...
def print_per_iteration(epoch, data_log_keys, loss_key_list, loss_log_dict, print_out_last):
    if print_out_last > 0:
        ...
        for k_loss in loss_key_list:
            ...
            for k_data in data_log_keys:
                if k_loss in loss_log_dict[k_data][0]:
                    ...
                    print('--{:<10s}-->'.format(k_data),
                          np.array2string(los_vec_cur_print, formatter={'float_kind': lambda x: "%8.3f" % x}))
...
def main(epochs=20, print_out_last=0,
    ...
    for epoch in range(ep_fr, epochs):
        ...
        print_per_iteration(epoch, data_log_keys, loss_key_list, loss_log_dict, print_out_last)
